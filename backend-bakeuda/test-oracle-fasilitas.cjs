...
async function run() {
  try {
    ...
    try {
      let result = await conn.execute("SELECT * FROM FASILITAS WHERE ROWNUM <= 5");
      console.log("FASILITAS table:", result.metaData.map(m => m.name));
      console.log("FASILITAS data:", result.rows);
    } catch(e) {
