...
class DetailTujuanInputDto {
  ...
  @IsOptional() @IsNumber() luas_tanah_baru?: number; // wajib untuk BARU/PECAH/PERUBAHAN_DATA/MUTASI — OPSIONAL untuk GABUNG (auto-dihitung dari total NOP asal, input diabaikan kalau diisi)
  @IsOptional() @IsNumber() luas_bangunan_baru?: number; // sama, auto-dihitung untuk GABUNG
  @IsEnum(JenisTanah) jenis_tanah_baru: JenisTanah;
  @IsOptional() @IsString() jalan_op_baru?: string; // untuk GABUNG: opsional, fallback ke alamat NOP asal pertama kalau kosong
...
|---|---|---|
| `BARU` | Kosong/tidak diisi | Wajib tepat **1** baris, `luas_tanah_baru` wajib diisi |
| `MUTASI` | Wajib tepat **1** baris | Wajib tepat **1** baris (isi minimal `nik_calon_subjek`, field lain boleh sama seperti data lama) |
| `PECAH` | Wajib tepat **1** baris | Wajib **≥2** baris, `luas_tanah_baru` wajib diisi tiap baris (soft warning kalau total beda dari luas asal — lihat `validasi-soft-warning-luas-tanah.md`) |
| `GABUNG` | Wajib **≥2** baris | Wajib tepat **1** baris — `luas_tanah_baru`/`luas_bangunan_baru` **opsional, diabaikan** (auto-hitung dari total NOP asal); `jalan_op_baru` **opsional** (fallback ke alamat NOP asal pertama kalau kosong) |
...
private async eksekusiGabung(tx: Prisma.TransactionClient, transaksi: TransaksiSpopWithDetail, currentUser: CurrentUser) {
  // 1. Ambil data lengkap semua NOP asal SEBELUM dinonaktifkan — dipakai untuk
  //    auto-sum luas dan fallback alamat.
  ...
  // (urutan sesuai yang diinput DESA saat submit), hanya dipakai kalau DESA
  // tidak mengisi field terkait secara eksplisit.
  ...
  const objekBaru = await tx.objekPajak.create({
    data: {
      nop, kode_wilayah: kodeWilayah, kode_blok: t.kode_blok_baru || '001',
      no_urut: nop.substring(13, 17), kode_jenis_op: '1',
...
3. ~~`GABUNG` — total luas tanah tujuan~~ — **Sudah diputuskan:** dihitung otomatis dari penjumlahan `luas_tanah`/`luas_bangunan` semua NOP asal di `eksekusiGabung()` (section 3.4), input manual DESA untuk field ini diabaikan. Alamat (`jalan_op_baru`) tetap bisa diisi manual, dengan fallback ke alamat NOP asal pertama kalau dikosongkan.
