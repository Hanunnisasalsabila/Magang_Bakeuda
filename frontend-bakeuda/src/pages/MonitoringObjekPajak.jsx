        ...
        const formattedList = rawList.flatMap(item => {
          ...
          return item.detail_tujuan.map(detail => {
            ...
            return {
              ...
              status: status,
              rawDate: new Date(item.tanggal_pengajuan),
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  <button
                    ...
                    className="w-[38px] h-[38px] rounded-lg bg-[#001f5c] text-white flex items-center justify-center shadow-sm hover:bg-[#001746] transition-colors focus:outline-none shrink-0"
                    title="Filter Urutan"
                  >
                    <span className="material-symbols-outlined text-[20px]">filter_list</span>
                  </button>
                  {showFilter && (
                    <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-lg shadow-lg border border-outline-variant py-2 z-10">
                      <button
                        onClick={() => { setSortOrder('newest'); setShowFilter(false); setCurrentPage(1); }}
                        className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 flex items-center justify-between ${sortOrder === 'newest' ? 'text-[#001f5c] font-bold' : 'text-gray-700'}`}
                      >
                        Urutkan (Terbaru - Terlama)
                        {sortOrder === 'newest' && <span className="material-symbols-outlined text-[18px]">check</span>}
                      </button>
                      <button
                        onClick={() => { setSortOrder('oldest'); setShowFilter(false); setCurrentPage(1); }}
                        className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 flex items-center justify-between ${sortOrder === 'oldest' ? 'text-[#001f5c] font-bold' : 'text-gray-700'}`}
                      >
                        Urutkan (Terlama - Terbaru)
                        {sortOrder === 'oldest' && <span className="material-symbols-outlined text-[18px]">check</span>}
                      </button>
                    </div>
                  )}
                </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container-low/50 text-on-surface-variant font-label-sm uppercase tracking-wider text-[11px]">
                ...
                <th className="px-4 py-3 font-bold border-b border-outline-variant text-left">Alamat Subjek</th>
              ...
              {loading ? (
                ...
                paginatedSubmissions.map((obj, i) => (
                  <tr key={obj.id_detail || `${obj.id}-${i}`} className="hover:bg-surface-container-low transition-colors border-b border-outline-variant/30">
                    <td className="px-4 py-3 text-center text-sm text-gray-500 font-medium w-[1%] whitespace-nowrap">{startIndex + i + 1}</td>
                    <td className="px-4 py-3 font-data-mono font-bold text-primary text-sm w-[1%] whitespace-nowrap">{obj.nop}</td>
                    <td className="px-4 py-3 text-base font-bold text-on-surface min-w-[150px]">{obj.name}</td>
                    <td className="px-4 py-3 text-sm text-on-surface-variant" title={obj.address}>{obj.address.length > 35 ? obj.address.substring(0, 20) + '...' : obj.address}</td>
                    <td className="px-4 py-3 text-base font-data-mono text-center w-[1%] whitespace-nowrap">{obj.land}</td>
                    <td className="px-4 py-3 text-base font-data-mono text-center w-[1%] whitespace-nowrap">{obj.building}</td>
                    <td className="px-4 py-3 text-sm font-data-mono text-center text-on-surface-variant w-[1%] whitespace-nowrap">{obj.date}</td>
            ...
            {Array.from({ length: Math.min(5, totalPages > 0 ? totalPages : 1) }, (_, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-8 h-8 rounded-md text-sm font-bold transition-all ${safeCurrentPage === pageNum
