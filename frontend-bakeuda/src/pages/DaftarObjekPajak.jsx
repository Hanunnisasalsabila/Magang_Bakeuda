...
export default function DaftarObjekPajak() {
  ...
  const filteredObjects = objects.filter((obj) => {
    ...
    const matchesStatus =
      ...
      (statusVerif === 'Terverifikasi' && obj.status === 'Verifikasi') ||
      (statusVerif === 'Draft' && obj.status === 'Draft') ||
      (statusVerif === 'Ditolak' && obj.status === 'Ditolak');
    const matchesSearch =
    ...
    <main className="p-gutter max-w-screen-2xl mx-auto w-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => alert('Data objek pajak berhasil diekspor!')}
          ...
          <button
            onClick={() => alert('Fitur tambah objek pajak baru.')}
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-primary text-on-primary font-section-header">
                <th className="px-6 py-4 uppercase text-[12px] tracking-widest border-b border-primary/20">
                  NOP
                </th>
                <th className="px-6 py-4 uppercase text-[12px] tracking-widest border-b border-primary/20">
                  Subjek Pajak
                ...
                <th className="px-6 py-4 uppercase text-[12px] tracking-widest border-b border-primary/20 text-center">
                  Tanah (m²)
                </th>
                <th className="px-6 py-4 uppercase text-[12px] tracking-widest border-b border-primary/20 text-center">
                  Bgn (m²)
                ...
                <th className="px-6 py-4 uppercase text-[12px] tracking-widest border-b border-primary/20 text-right">
                  Aksi
                </th>
              </tr>
            </thead>
        ...
        <div className="px-6 py-4 border-t border-outline-variant flex flex-col sm:flex-row items-center justify-between gap-4 bg-surface-container-low/20">
          <div className="text-sm text-on-surface-variant">
            Menampilkan <span className="font-bold text-on-surface">1 - {filteredObjects.length}</span> dari{' '}
            <span className="font-bold text-on-surface">{filteredObjects.length}</span> data objek pajak
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6">
        <div className="bg-primary/5 border border-primary/10 rounded-xl p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-primary uppercase tracking-widest">Total Objek Pajak</p>
        ...
        <div className="bg-secondary/5 border border-secondary/10 rounded-xl p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-secondary uppercase tracking-widest">Terverifikasi</p>
        ...
        <div className="bg-error/5 border border-error/10 rounded-xl p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-error uppercase tracking-widest">Menunggu Review</p>
