...
export default function Sidebar({ role, activePage, onNavigate, isOpen, onClose }) {
  ...
  const menuItems = isDesa
    ? [
        { id: 'dashboard_desa', label: 'Dasbor Utama', icon: 'dashboard' },
    ...
    : [
        { id: 'dashboard_admin', label: 'Dasbor Utama', icon: 'dashboard' },
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed left-0 top-0 h-screen w-64 bg-white border-r border-gray-200 py-6 flex flex-col z-50 transition-transform duration-300 md:translate-x-0 ${
