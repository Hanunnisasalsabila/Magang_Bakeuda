    ...
    return Column(
      ...
      children: [
        Row(children: [
            ...
            child: DropdownButtonFormField<String>(
              ...
              onChanged: (v) {
                updateFormState(() {
                   ...
                   } else if (_jmlBangunanController.text == '0' || _jmlBangunanController.text.isEmpty) {
          ...
          Expanded(child: CustomTextField(controller: _luasTanahController, label: 'Luas (m²) *', keyboardType: TextInputType.number, validator: (v) {
        ...
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: CustomTextField(controller: _rtOpController, label: 'RT *', keyboardType: TextInputType.number, inputFormatters: [LengthLimitingTextInputFormatter(3), FilteringTextInputFormatter.digitsOnly], validator: (v) => v == null || v.isEmpty ? 'Wajib' : null, hintText: '001')),
          const SizedBox(width: 8),
          Expanded(child: CustomTextField(controller: _rwOpController, label: 'RW *', keyboardType: TextInputType.number, inputFormatters: [LengthLimitingTextInputFormatter(3), FilteringTextInputFormatter.digitsOnly], validator: (v) => v == null || v.isEmpty ? 'Wajib' : null, hintText: '001')),
          const SizedBox(width: 8),
          Expanded(flex: 2, child: DropdownButtonFormField<String>(
            value: _getValidKecamatan(_kecamatanOpController.text),
            isExpanded: true,
            decoration: InputDecoration(labelText: 'Kecamatan Objek *', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Colors.grey))),
            ...
            onChanged: _isOpWilayahPatented ? null : (v) => updateFormState(() {
        ...
        Row(children: [
          Expanded(child: DropdownButtonFormField<String>(
            value: _getValidKelurahan(_kecamatanOpController.text, _kelurahanOpController.text),
            isExpanded: true,
            decoration: InputDecoration(labelText: 'Kelurahan Objek *', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Colors.grey))),
            items: _getValidKecamatan(_kecamatanOpController.text) != null
                ? WilayahData.data.where((e) => e['kecamatan'] == _getValidKecamatan(_kecamatanOpController.text)).map((e) => e['nama_desa']!).toSet().toList().map((e) => DropdownMenuItem(value: e, child: Text(e, style: const TextStyle(fontSize: 14), overflow: TextOverflow.ellipsis))).toList()
                : [],
            onChanged: _isOpWilayahPatented ? null : (v) => updateFormState(() => _kelurahanOpController.text = v ?? ''),
            validator: (v) => v == null || v.isEmpty ? '*' : null,
          )),
          const SizedBox(width: 12),
          Expanded(child: CustomTextField(controller: _blokKavController, label: 'Blok/Kav/No', inputFormatters: [LengthLimitingTextInputFormatter(10)])),
        ]),
        const SizedBox(height: 12),
            ...
            children: [
              ...
              Positioned(
                ...
                child: FloatingActionButton.small(
                  onPressed: () => updateFormState(() => _isSatellite = !_isSatellite),
          ...
          children: [
            Expanded(
              child: ElevatedButton.icon(
                onPressed: _polygonPoints.isEmpty ? null : () => updateFormState(() {
            ...
            child: ClipRRect(
              ...
              child: SingleChildScrollView(
                ...
                child: DataTable(
                  ...
                  rows: _polygonPoints.asMap().entries.map((e) => DataRow(cells: [
        ...
        const SizedBox(height: 12),
        if (_polygonPoints.isNotEmpty) ...[
          Text('Lihat di Peta Eksternal:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
          const SizedBox(height: 8),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: [
              ...
              ElevatedButton.icon(
                onPressed: () async {
                  ...
                  if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Koordinat disalin!'), backgroundColor: Colors.green));
